// Lasso (Laplace) variant of paper_model_PPOM. Replaces the normal(0, 5)
// prior on beta_variant with double_exponential(0, 1). Architecture is
// otherwise identical: cutpoints estimated as parameters with N(log2(MIC),
// 0.5) prior, scalar lineage-level heritability, raw variant_matrix in
// linear predictor (no centering), no alpha_mean.
...
model {
  ...
  to_vector(beta_variant) ~ double_exponential(0, 1);
