// Lasso (Laplace) prior on per-cutpoint variant effects, with raw genotype X.
// Derived from final_no_horseshoe_no_centering_PPOM.stan: identical architecture
// (alpha_mean, fixed cutpoints from log2(mic_breakpoints), lineage handling,
// raw genotype matrix) except the prior on beta_variant_raw is
// double_exponential(0, 1) instead of normal(0, 1). Adds deterministic-subset
// PPC (y_rep_ppc/y_true_ppc) and per-cutpoint heritability metrics in
