// Lasso (Laplace) version of POM_prediction.stan -- proportional-odds ordinal
// logistic model (POM) for phenotype prediction.
//
...
//
// Differs from POM_prediction.stan in exactly one place: the regularized
...
parameters {
  ...
  // Lasso (Laplace) shrinkage on variant effects (single effect per variant shared across cutpoints)
...
generated quantities {
  ...
  // Some tips on interpreting these:
    ...
    // Lasso shrinkage biases V_A downward, so h2_narrow is a lower bound
