// Proportional-odds ordinal (POM) prediction model -- training/test split
// counterpart of POM_inference.stan.
//
// Fits on N_train samples with the same likelihood, priors, fixed cutpoints
// (log2(mic_breakpoints)), and generated summaries as the inference model,
// and emits predicted_phenotype on N_test held-out samples as integer ordinal
// draws. gwas_workflow/R/prediction.R tabulates these into K-column frequency
// probabilities for compute_ordinal_accuracy(). No PPC outputs.
...
transformed data {
  ...
  // Empirical baseline from training only: Laplace-smoothed reference-sublineage
  // fraction in MIC category 1. Matches POM_inference.stan.
  int n_ref = 0;
  int n_ref_cat1 = 0;
  for (n in 1:N_train) {
...
generated quantities {
  ...
  // Held-out integer ordinal draws consumed by
  // gwas_workflow/R/prediction.R (tabulated to K-column frequency probabilities
  // for compute_ordinal_accuracy()).
