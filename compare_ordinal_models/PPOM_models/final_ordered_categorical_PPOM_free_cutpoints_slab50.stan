// DIAGNOSTIC PPOM variant: free_cutpoints with WIDER horseshoe slab.
//
// Identical to final_ordered_categorical_PPOM_free_cutpoints.stan EXCEPT
// slab_scale is widened from 5 -> 50. cutpoint_prior_sd stays at 0.25.
//
// Motivation: probes whether the slab_scale = 5 cap is shrinking late-k
// variant effects that genuinely exceed |beta_std| ~ 5. With slab_scale = 50,
// the regularised horseshoe slab admits effects up to ~50 standardised units
// before biting -- still tighter than tight_alpha_tau1's slab=200 (which is
// effectively non-informative) but 10x looser than tight_slab/free_cutpoints.
// This isolates the slab-cap effect from the cutpoint-drift fix tested in
// the _wide_drift sibling.
//
// If this variant inflates similarly to the original free_cutpoints, the
// slab cap was not the binding constraint. If it inflates more (to a point
// between the original free_cutpoints and tight_alpha_tau1), the cap was
// regularising late-k effects and slab=5 is the better choice. If it
// inflates less, the cap was over-shrinking real effects.
...
transformed parameters {
  ...
  // CHANGE vs free_cutpoints: 5 -> 50 (looser horseshoe slab cap)
  real slab_scale = 50;
  real nu         = 4;
  real tau_0      = 1;
