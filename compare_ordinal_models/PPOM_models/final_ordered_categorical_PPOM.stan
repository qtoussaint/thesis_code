// Partial proportional odds (PPOM) version of final_ordered_categorical_POM.stan.
...
// Sublineage effects and the fixed intercept remain proportional across cutpoints.
...
generated quantities {
  ...
  vector[K] cat_freq_rep = rep_vector(0, K);
  {
    ...
    for (n in 1:N) {
      // Per-threshold cumulative P(y <= k) with cutpoint-specific variant effects.
      // Cumulative may be non-monotone under PPOM; clamp differences to >= 0 and renormalize.
      vector[K-1] cdf_n;
      for (k in 1:(K-1)) {
