...
// AND WIDER horseshoe slab. Combines _wide_drift and _slab50 changes.
//
// Identical to final_ordered_categorical_PPOM_free_cutpoints.stan EXCEPT:
//   cutpoint_prior_sd: 0.25 -> 1.5
//   slab_scale       : 5    -> 50
//
// Motivation: tests the interaction of the two diagnostic changes. If
// _wide_drift alone collapses inflation but _slab50 alone does not, this
// combo should match _wide_drift. If _slab50 alone increases inflation,
// this combo lets the wider drift absorb more of the misfit and may still
// match _wide_drift -- confirming the cutpoint freedom is doing the work
// and the slab choice is secondary in its presence.
...
transformed data {
  ...
  // CHANGE vs free_cutpoints: 0.25 -> 1.5 (matches _wide_drift sibling)
...
transformed parameters {
  ...
  // CHANGE vs free_cutpoints: 5 -> 50 (matches _slab50 sibling)
  real slab_scale = 50;
  real nu         = 4;
  real tau_0      = 1;
...
generated quantities {
  ...
  // Per-cutpoint drift vs MIC-grid anchor.
