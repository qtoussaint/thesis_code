// DIAGNOSTIC PPOM variant: free_cutpoints + wide cutpoint drift + LOOSER tau
// prior + RAISED slab ceiling.
//
// Identical to final_ordered_categorical_PPOM_free_cutpoints_wide_drift.stan
// EXCEPT tau_0 is widened from 1 -> 5 AND slab_scale is raised from 5 -> 50.
// cutpoint_prior_sd stays at 1.5.
//
// Motivation: posterior medians of beta_variant[v, k] in wide_drift come out
// smaller than expected. This variant pulls both regularised-horseshoe levers
// at once: (a) tau_0 = 5 lets the global scale tau sit higher when the data
// warrant, raising every effect's effective scale through the multiplicative
// tau * lambda_tilde structure; (b) slab_scale = 50 raises the cap that the
// slab places on individual effects, so non-null variants whose
// lambda_variant[v, k] does push into the tail are no longer truncated near
// 5. Combining both is the genuinely new configuration not covered by any
// existing sibling --- wide_drift_slab50 raises only (b) and there is no
// existing free_cutpoints variant that varies tau_0.
//
// Read in conjunction with the tau5-only sibling: if tau5 alone is enough,
// the global scale was the binding constraint; if only this combined variant
// produces materially larger effects, both shrinkage levers were binding.
// If even this variant does not raise effect sizes, the binding constraint
// is mechanism (1) --- per-cutpoint absorption with cutpoint_prior_sd = 1.5
// --- and the next variant should tighten the cutpoint prior or
// reparameterise into level + spacings.
...
transformed data {
  ...
  // Inherited from wide_drift: 0.25 -> 1.5 to admit the implied non-uniform misfit
...
transformed parameters {
  ...
  real slab_scale = 50;  // CHANGE vs wide_drift: 5 -> 50 to raise effect cap
  real nu         = 4;
  real tau_0      = 5;   // CHANGE vs wide_drift: 1 -> 5 to loosen global shrinkage
