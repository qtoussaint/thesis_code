#!/usr/bin/env Rscript
# Log-scaled version of faceted_rate_beta_summary.R: identical layout, but the y-axis
# is log10(RATE) or log10(|median beta|) instead of the raw value. Axis/strip labels are
# the matching log[10] plotmath. Non-positive values (log10 -> -Inf) and NAs are dropped.
#
# Per species (tb_rifampicin, spn_penicillin, spn_trimethoprim):
#   <species>_POM_faceted_RATE_beta.png  -- log10(|median beta|) (top row) and log10(RATE)
#                                           (bottom row), one column per binning strategy
#   <species>_PPOM_faceted_RATE.png      -- per-binning blocks stacked vertically, each
#                                           faceted by cutpoint, log10(RATE) on y
#   <species>_PPOM_faceted_beta.png      -- same blocks, log10(|median beta|) on y
# Plus one figure across all species:
#   logistic_all_species_faceted_RATE_beta.png -- logistic log10(|median beta|) (top) and
#                                                  log10(RATE) (bottom), one column per species
#
...
#   Rscript paper_figures/faceted_rate_beta_summary_logscaled.R
#
# Output: <RES>/paper_figures/faceted_rate_beta/logscaled/
...
# build_df, build_faceted_plot. build_faceted_plot is overridden below for log10 scaling.
...
OUT_DIR    <- file.path(RES, "paper_figures", "faceted_rate_beta", "logscaled")
NCOL       <- 2L
...
# plotmath strings (parsed in the facet strips): log10 of |beta-tilde| and of the plain
# RATE label.
METRIC_LABELS <- c(beta = "log[10]*group('|', tilde(beta), '|')",
                   rate = "log[10]~'relative centrality (RATE)'")
metric_labeller <- ggplot2::as_labeller(METRIC_LABELS, ggplot2::label_parsed)

# Expression form of the log10 metric labels for direct ylab() use (cowplot panels).
LOG10_YLAB <- list(
  beta = expression(log[10] * group("|", tilde(beta), "|")),
  rate = expression(log[10] ~ "relative centrality (RATE)"))

# log10, with non-positive / non-finite inputs collapsed to NA so those rows drop out.
...
# Override the sourced build_faceted_plot so the PPOM path plots log10(value) with the
# matching log[10] axis title. Otherwise identical to the helper in
# replot_rate_faceted_cutpoints.R.
build_faceted_plot <- function(run_dir, annotations, genes_of_interest = NULL,
  ...
  df$value <- log10_safe(if (metric == "abs_beta") abs(df$median) else df$rate)
  df <- df[!is.na(df$value), ]
  ylab <- if (metric == "abs_beta") LOG10_YLAB$beta else LOG10_YLAB$rate
...
# with y on the log10 scale. Rows whose log10(value) is non-finite (value <= 0 or NA) drop
# out so their facet panel stays empty.
to_long_metric <- function(df) {
  long <- rbind(
    transform(df, metric = "beta", value = log10_safe(abs(median))),
    transform(df, metric = "rate", value = log10_safe(rate)))
...
build_pom_wrapped <- function(long, label_df) {
  ...
  beta_p <- panel("beta", LOG10_YLAB$beta)
  rate_p <- panel("rate", LOG10_YLAB$rate)
