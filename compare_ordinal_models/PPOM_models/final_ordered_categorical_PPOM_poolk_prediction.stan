...
// final_ordered_categorical_PPOM_poolk.stan.
//
// Same training/test architecture as gwas_finalmodels/PPOM_prediction.stan,
// but with this diagnostic variant's structure: per-variant horseshoe anchor
// z_base[v] (slab_scale = 5, tau_0 = 1) plus per-(v, k) deviation
// sigma_dev * z_dev[v, k]. Fits on N_train samples and emits an N_test x K
// probability matrix `predicted_phenotype` (Rao-Blackwellized; no categorical
// RNG) consumed by gwas_workflow/R/prediction.R via
