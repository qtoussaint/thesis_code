#!/usr/bin/env Rscript
# Compose one PPOM paper figure (Spn trimethoprim) from a manhattan_short
# directory and a locus-zoom directory.
#
# Usage:
#   Rscript manhattan_locuszoom_composite_trimethoprim.R <manhattan_dir> <locuszoom_dir> [--output-dir <dir>] [--analysis-name <name>]
#
# Output: <output_dir>/figure_<analysis_name>_trimethoprim.png
...
parse_args <- function(argv) {
  ...
  if (length(positional) != 2) {
    stop("Usage: Rscript manhattan_locuszoom_composite_trimethoprim.R <manhattan_dir> <locuszoom_dir> [--output-dir <dir>] [--analysis-name <name>]")
  }
  list(
...
main <- function() {
  ...
  output_path <- file.path(args$output_dir, paste0("figure_", analysis_name, "_trimethoprim.png"))
