...
parameters {
  ...
  ordered[K-1] cutpoints; // estimated cutpoints for ordinal regression
...
model {
  ...
  // estimated cutpoints with informative prior centered on log2(MIC breakpoints)
  ...
  mu = (variant_matrix * beta_variant) + (sublineages_treatmentcontrast * beta_sublineage);
...
generated quantities {
  ...
  for (v in 1:V) {
    beta_variant_allele[v] = beta_variant[v];          // already on 0->1 allele scale
