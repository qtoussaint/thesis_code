// Binary logistic association model, lineage + subcluster population structure control
//
// Companion to logistic_lineage_inference.stan in this directory. Everything outside
// the population-structure term is byte-identical across the pair and with
// gwas_finalmodels/logistic_inference.stan -- same genotype standardization, same
...
// are directly comparable between runs.
//
// This model adds the subcluster level on top of the lineage covariate, so the
// lineage-only model is strictly nested inside it and the pair isolates what the
// subcluster level contributes.
//
// Lineage enters as a covariate with treatment-contrast encoding (reference lineage =
// column 1 of lineage_matrix). Subclusters enter as within-lineage deviations with
// sum-to-zero centering. In gwas_finalmodels/logistic_inference.stan the subcluster
// hyper-mean was beta_lineage[parent_lineage_sub], but the within-lineage centering
// that follows subtracts it again exactly, so that term is dropped here: the lineage
// level is carried by X_lineage instead, where it reaches the likelihood.
//
...
transformed data {
  // Treatment-contrast encoding: drop the reference lineage and the reference
  // sublineage (column 1 of each)
...
transformed parameters {
  // Non-centred subcluster deviations. No lineage hyper-mean here: the lineage
  // level is a covariate in the linear predictor, and any constant added within a
  // lineage would be removed by the centering below anyway.
  vector[S-1] beta_sublineage_raw = sigma_sublineage * z_sub;
