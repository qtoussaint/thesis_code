// Partial proportional-odds (PPOM) prediction model -- training/test split
// counterpart of PPOM_inference.stan.
//
// Fits on N_train samples with the same likelihood, priors, fixed cutpoints,
// and generated summaries as the inference model, and emits predicted_phenotype
// on N_test held-out samples as an N_test x K probability matrix
// (Rao-Blackwellized: the per-category probability is analytically available
// at every draw, so no categorical RNG is used). gwas_workflow/R/prediction.R
// reads this column-major-flattened layout via
//   idx <- s + (N_test * 0:n_cutpoints)
// for compute_ordinal_accuracy(). No PPC outputs.
...
generated quantities {
  ...
  // Held-out per-category probabilities consumed by
  // gwas_workflow/R/prediction.R -> compute_ordinal_accuracy().
  // Rao-Blackwellized: probability is analytically available at each draw,
  // no categorical RNG step.
  ...
  // Per-cutpoint liability-scale heritability, computed on the training partition.
