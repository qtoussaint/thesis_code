// Lasso (Laplace) variant of final_ordered_categorical_PPOM_tight_alpha_tau1,
// using raw genotype X (no centering/scaling). Replaces the regularized
// horseshoe on beta_variant with a double_exponential(0, 1) prior.
// Architecture: fixed cutpoints at log2(mic_breakpoints), free alpha under
// data-empirical tight Gaussian prior anchored on p_baseline_emp, raw
...
model {
  to_vector(beta_variant_raw) ~ double_exponential(0, 1);
...

  {
    ...
    for (n in 1:N) {
      for (k in 1:(K-1)) {
        real mu_nk = alpha + dot_product(variant_matrix[n], beta_variant_raw[, k]) + sub_eta[n];
        if (phenotype[n] <= k) {
...
generated quantities {
  ...
  array[N_ppc] int y_true_ppc;
  {
    ...
    for (i in 1:N_ppc) {
      ...
      vector[K-1] cdf_n;
      for (k in 1:(K-1)) {
        real mu_nk = alpha + dot_product(variant_matrix[n], beta_variant_raw[, k]) + sub_eta_gen[n];
