// DIAGNOSTIC PPOM variant: free_cutpoints + wide cutpoint drift + LOOSER tau prior.
//
// Identical to final_ordered_categorical_PPOM_free_cutpoints_wide_drift.stan
// EXCEPT tau_0 is widened from 1 -> 5. cutpoint_prior_sd stays at 1.5 and
// slab_scale stays at 5.
//
// Motivation: posterior medians of beta_variant[v, k] in wide_drift come out
// smaller than expected. Two suspects: (i) wide cutpoints absorbing per-k
// variant signal, (ii) the regularised horseshoe global scale tau being too
// tightly anchored. tau ~ cauchy(0, tau_0=1) concentrates tau near 0 in the
// usual horseshoe regime, so non-null variant effects must push their local
// lambda_variant[v, k] into the tail to escape shrinkage, where the slab
// (slab_scale=5) caps them. Loosening the global scale to tau_0 = 5 lets
// tau itself sit higher when the data warrant, raising every effect's
// effective scale via the multiplicative tau * lambda_tilde structure
// without giving up the horseshoe sparsity at the local-scale level.
//
// If this variant materially raises median |beta_variant| relative to
// wide_drift, the global shrinkage was the binding constraint. If tau5
// alone is not enough but tau5 + slab50 is, both shrinkage levers were
// binding. If neither helps, the binding constraint is mechanism (1) ---
// per-cutpoint absorption --- and the next variant should tighten
// cutpoint_prior_sd (e.g. 0.5) or reparameterise into level + spacings.
