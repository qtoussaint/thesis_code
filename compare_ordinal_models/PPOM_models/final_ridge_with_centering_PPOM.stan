// Ridge (Gaussian, sigma=5) prior on per-cutpoint variant effects, with
// centered+scaled X. Derived from final_no_horseshoe_with_centering_PPOM.stan:
// identical architecture (alpha_mean, fixed cutpoints from
// log2(mic_breakpoints), lineage handling) except the prior on
// beta_variant_std is normal(0, 5) instead of normal(0, 1). Adds
// deterministic-subset PPC (y_rep_ppc/y_true_ppc) and per-cutpoint
// heritability metrics in generated quantities.
...
model {
  ...
  to_vector(beta_variant_std) ~ normal(0, 5);
