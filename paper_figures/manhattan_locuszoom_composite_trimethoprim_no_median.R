#!/usr/bin/env Rscript
# Compose one PPOM paper figure (Spn trimethoprim, without the median_effects
# manhattan row) from a manhattan_short directory and a locus-zoom directory.
#
# Usage:
#   Rscript manhattan_locuszoom_composite_trimethoprim_no_median.R <manhattan_dir> <locuszoom_dir> [--output-dir <dir>] [--analysis-name <name>]
#
# Output: <output_dir>/figure_<analysis_name>_trimethoprim_no_median.png
...
parse_args <- function(argv) {
  ...
  if (length(positional) != 2) {
    stop("Usage: Rscript manhattan_locuszoom_composite_trimethoprim_no_median.R <manhattan_dir> <locuszoom_dir> [--output-dir <dir>] [--analysis-name <name>]")
  }
  list(
...
main <- function() {
  ...
  output_path <- file.path(args$output_dir, paste0("figure_", analysis_name, "_trimethoprim_no_median.png"))
