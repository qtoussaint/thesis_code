...
// final_ordered_categorical_PPOM_free_cutpoints_wide_drift_lasso_estscale_mixture_no_centering.stan
...
transformed parameters {
  ...
  for (k in 1:(K-1))
    for (v in 1:V)
      beta_variant_raw[v, k] = z_variant[v, k] * sqrt(tau2_local[v, k]);
...
generated quantities {
  ...
  real<lower=0, upper=1> h2_broad_median_k;
  {
    ...
    for (k in 1:(K-1)) {
      vector[N_train] g_variant_k = training_variants * beta_variant_raw[, k];
