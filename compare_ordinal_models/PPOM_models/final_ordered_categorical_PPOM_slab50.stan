// DIAGNOSTIC PPOM variant: identical to final_ordered_categorical_PPOM_tight_slab.stan
// EXCEPT slab_scale is widened from 5 -> 50. All other hyperparameters,
// priors, and likelihood identical (fixed cutpoints, free alpha).
//
// Sits as a middle data point on the slab-width axis between tight_slab
// (slab=5, strong shrinkage of late-k effects) and tight_alpha_tau1
// (slab=200, effectively non-informative slab). With slab=50, the
// regularised horseshoe slab admits effects up to ~50 standardised units
// before biting -- 10x looser than tight_slab, 4x tighter than the baseline.
//
// Use cases:
//   - If tight_slab over-shrunk genuine resistance variants, this should
//     show those variants escaping while still controlling rare-event
//     spurious inflation.
//   - Compared to tight_alpha_tau1 on the same datasets, isolates how much
//     of that baseline's inflation came from the slab cap being effectively
//     absent vs other mechanisms.
//
// Sanity checks produced in generated quantities mirror tight_slab so the
...
transformed parameters {
  ...
  // ONLY change vs tight_slab: slab_scale 5 -> 50
  real slab_scale = 50;
  real nu         = 4;
  real tau_0      = 1;
