// DIAGNOSTIC PPOM variant: free_cutpoints + wide cutpoint drift + TIGHT lambda.
//
// Identical to final_ordered_categorical_PPOM_free_cutpoints_wide_drift.stan
// EXCEPT lambda_variant's Cauchy prior is tightened from scale 2 -> 1.
// tau is still sampled (tau ~ cauchy(0, tau_0=1)) and slab_scale stays at 5.
//
// Motivation: isolates the effect of the lambda prior alone, holding the
// global-shrinkage (sampled tau) and slab choices fixed at the wide_drift
// baseline. Pairs with the fixed-tau x slab sweep (which keeps lambda at
// Cauchy(0, 1)) to attribute posterior differences to each knob
// independently. A clean A/B contrast: wide_drift vs wide_drift_lambda1
// is the lambda-only comparison.
...
model {
  ...
  to_vector(lambda_variant) ~ cauchy(0, 1);  // CHANGE vs wide_drift: 2 -> 1
