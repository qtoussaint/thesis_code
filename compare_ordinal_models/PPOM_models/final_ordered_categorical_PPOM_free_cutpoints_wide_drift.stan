// DIAGNOSTIC PPOM variant: free_cutpoints with WIDENED cutpoint-drift prior.
//
// Identical to final_ordered_categorical_PPOM_free_cutpoints.stan EXCEPT
// cutpoint_prior_sd is widened from 0.25 -> 1.5. slab_scale stays at 5.
//
// Motivation: cross-dataset diagnostics (mechanism-3 analysis) showed
// latent_scale's posterior sigma_latent reached ~5 on dataset 11 (~13 prior-SDs
// above its anchor of 1), implying the latent-scale misfit is on the order of
// log2(5) ~= 2.3 latent units. The original free_cutpoints prior of N(anchor, 0.25)
// admits drift of only +/- 0.5 log2 at 95% coverage, ~10x too tight for the
// implied misfit. With cutpoint_prior_sd = 1.5, each cutpoint can drift
// +/- 3 log2 units at 95% coverage, comfortably covering the implied
// non-uniform spacing misfit while still regularising cutpoint identification.
//
// If this variant collapses inflation_ratio_max_abs on the fine-dilution
// datasets (02 and 10) where the original free_cutpoints did not, the prior
// width was the binding constraint and mechanism (3) is fully accommodated by
// per-cutpoint drift. If it doesn't help, either the misfit is even larger
// (try wider_drift at SD ~ 5) or some other source of inflation remains.
...
transformed data {
  ...
  // CHANGE vs free_cutpoints: 0.25 -> 1.5 to admit the implied non-uniform misfit
