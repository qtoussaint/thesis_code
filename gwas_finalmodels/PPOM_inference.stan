// Hybrid PPOM: free cutpoints (wide-drift parameterization) + production-style
// horseshoe and diagnostics.
//
// Differs from fixed_cutpoints_parameterization/PPOM_inference.stan in three
// ways: (1) the fixed log2(mic_breakpoints) cutpoints are replaced by an
// estimated ordered[K-1] cutpoints with a wide normal prior anchored on a
// shifted MIC ladder, (2) the free alpha intercept is removed (its level shift
// is folded into the cutpoint prior mean), and (3) horseshoe hyperparameters
// are retuned to slab_scale = 5, tau_0 = 1 to match the diagnostic variant.
//
// Diagnostics retained / added: beta_variant_std_prior (post-hoc prior check on
// horseshoe-shrunk variant effects, carried over from the fixed model) and
// cutpoint_drift (per-cutpoint deviation from the MIC-grid anchor, carried
// over from the wide-drift model).
//
// All other structure is inherited from the fixed model: data-informed
// alpha_prior_mean used to shift the cutpoint anchor, treatment-contrast
// sublineage encoding with within-lineage sum-to-zero, partial proportional
// odds (independent horseshoe scale per (variant, cutpoint)).
...
transformed data {
  ...
  // MIC-grid cutpoint anchors on the log2 scale (used to build the cutpoint
  // prior; the cutpoints themselves are now parameters, not constants).
  ...
  // Shift the MIC-grid anchors down by alpha_prior_mean so cutpoint 1 sits
  // near logit(p_baseline_emp). This replaces the explicit alpha intercept:
  // the level shift is absorbed into the cutpoint prior while MIC-ladder
  // spacing is preserved.
  vector[K-1] cutpoint_prior_mean;
  for (k in 1:(K-1))
    cutpoint_prior_mean[k] = mic_cutpoints[k] - alpha_prior_mean;

  // Wide drift prior: SD = 1.5 admits ~+/- 3 log2 units of per-cutpoint drift
  // at 95%, comfortably covering the implied non-uniform latent-scale misfit
  // (mechanism-3) while still regularising cutpoint identification.
  real cutpoint_prior_sd = 1.5;
}

parameters {
  // Estimated cutpoints (replacing fixed cutpoints + free alpha). The
  // ordered[K-1] type enforces strict ordering; cutpoint_prior_mean /
  // cutpoint_prior_sd shape location within that constraint.
...
transformed parameters {
  ...
  // with treatment-contrast encoding and makes the cutpoint anchor uninterpretable)
  ...
  // Retuned vs the fixed-cutpoints model: slab_scale 200 -> 5 (real slab,
  // not effectively-off) and tau_0 0.1 -> 1 (wider global half-Cauchy).
  real slab_scale = 5;
  real nu         = 4;
  real tau_0      = 1;
...
model {
  ...
  // Wide normal prior on each cutpoint around its shifted MIC-grid anchor.
  // The ordered[K-1] type still enforces ordering; this prior shapes each
  // cutpoint's location within that constraint and absorbs any level shift
  // that would otherwise be carried by a free alpha intercept.
  ...
  // PPOM likelihood -- one bernoulli per (sample, cutpoint). No alpha:
  // the intercept is folded into the cutpoints.
  {
...
generated quantities {
  ...
  // pinpoint categories where the MIC anchor is wrong.
