...
mb_add_labels <- function(p, df, annotations, goi, mode, genes, n_labels,
  ...
  # headroom above the tallest peak + a small upward nudge so the label sits on
  # top of the peak SNP rather than clipping the panel edge
  rng <- range(df$value, na.rm = TRUE)
  nudge <- 0.04 * diff(rng)
  p +
    ggplot2::scale_y_continuous(expand = ggplot2::expansion(mult = c(0.05, 0.16))) +
    repel_label_layer(label_df, size = size, nudge_y = nudge)
}
