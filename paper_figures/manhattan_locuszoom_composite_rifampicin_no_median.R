#!/usr/bin/env Rscript
# Compose one binary-GWAS paper figure (TB rifampicin, without the median_effects
# manhattan row) from a manhattan_short directory and a locus-zoom directory.
#
# Binary GWAS runs produce a single manhattan per metric (no per-cutpoint
# overlay), so the manhattan filenames are manhattan_<metric>.png rather than
# manhattan_all_cutpoints_overlayed_<metric>.png.
#
# Usage:
#   Rscript manhattan_locuszoom_composite_rifampicin_no_median.R <manhattan_dir> <locuszoom_dir> [--output-dir <dir>] [--analysis-name <name>]
#
# Output: <output_dir>/figure_<analysis_name>_rifampicin_no_median.png
...
parse_args <- function(argv) {
  ...
  if (length(positional) != 2) {
    stop("Usage: Rscript manhattan_locuszoom_composite_rifampicin_no_median.R <manhattan_dir> <locuszoom_dir> [--output-dir <dir>] [--analysis-name <name>]")
  }
  list(
...
main <- function() {
  ...
  panel_a_path <- require_file(file.path(manhattan_dir, "manhattan_exp_abs_median.png"))
  panel_c_path <- require_file(file.path(manhattan_dir, "manhattan_RATE.png"))

  lz_genes <- c("rpoA", "rpoB", "rpoC")
  ...
  output_path <- file.path(args$output_dir, paste0("figure_", analysis_name, "_rifampicin_no_median.png"))
