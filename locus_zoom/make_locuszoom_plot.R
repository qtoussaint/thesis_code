...
# RATE_values.txt has a 3-line header; one RATE value per line after that.
# Detect how many header lines to skip by looking for non-numeric lines.
raw_lines <- readLines(opt$rate_file)
first_numeric <- which(suppressWarnings(!is.na(as.numeric(raw_lines))))[1]
n_skip <- first_numeric - 1L

rate_values <- as.numeric(raw_lines[first_numeric:length(raw_lines)])
message("  Loaded ", length(rate_values), " RATE values (skipped ", n_skip, " header lines)")
...
pos_df <- fread(opt$positions_file, header = FALSE)
positions <- as.integer(pos_df[[2]])

if (length(positions) != length(rate_values)) {
  stop(
    "Mismatch: ", length(rate_values), " RATE values but ",
    length(positions), " positions. These must be the same length."
  )
}
...
# ---------------------------------------------------------------------------
message("Loading ensembldb: ", opt$ensdb)
ensdb <- EnsDb(opt$ensdb)
