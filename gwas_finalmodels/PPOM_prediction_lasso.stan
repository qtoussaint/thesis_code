// Lasso (Laplace) version of PPOM_prediction.stan for phenotype prediction.

// Fits on N_train samples with the same likelihood, priors, free cutpoints, and
// generated summaries as the lasso inference model, then generates predicted_phenotype on
...
// Differs from PPOM_prediction.stan in exactly one place: the regularized
// horseshoe on beta_variant_std is replaced by a double_exponential(0, 1)
// (Laplace / lasso) prior. beta_variant_std stays a V x (K-1) matrix with an
// independent coefficient per (variant, cutpoint), so proportional odds is
// still relaxed for variant effects only. Lineage and sublineage effects
// remain proportional across cutpoints.
...
parameters {
  ...
  // Lasso (Laplace) shrinkage, independent per (variant, cutpoint)
...
model {
  // Double-exponential (Laplace) lasso prior, independent per (variant, cutpoint)
...
generated quantities {
  ...
  // Some tips on interpreting these:
    ...
    // Lasso shrinkage biases V_A_k downward, so h2_narrow_k is a lower bound
