// DIAGNOSTIC PPOM variant: free_cutpoints + wide cutpoint drift + LOGNORMAL-anchored tau.
//
// Identical to final_ordered_categorical_PPOM_free_cutpoints_wide_drift.stan EXCEPT
// the global horseshoe scale tau is given a tight lognormal prior instead of
// half-Cauchy(0, 1).
//
// Motivation: under tau ~ cauchy(0, 1), the data collapses tau to ~5e-4 and
// every variant ends up deep in the spike (top |beta| ~ 0.08 in this setup),
// crushing real weak signal. The sibling _fixedtau05_slab3 model pins tau at
// 0.05 and recovers more signal (top |beta| ~ 0.25, better predictive
// performance) but at the cost of losing adaptivity and adding false positives.
//
// lognormal(log(0.02), 0.3) anchors tau near 0.02 (mode), with ~95% mass
// roughly in [0.011, 0.036]. The data can still move tau within this band but
// cannot drive it to the 5e-4 collapse. At tau = 0.02, an "off" variant
// (lambda ~ 1) has spike width 2 sigma ~ +/- 0.04 -- tighter than the
// fixed-tau05 spike (+/- 0.1) but loose enough that borderline lambda values
// produce visible effects.
//
// slab_scale stays at 5 and lambda_variant stays at Cauchy(0, 2) (inherited
// from the wide_drift base); only the tau prior changes.
...
model {
  ...
  // Anchored tau: mode 0.02, ~95% mass in [0.011, 0.036]. Prevents the
  // half-Cauchy collapse to ~5e-4 while still letting the data move tau.
  tau ~ lognormal(log(0.02), 0.3);
