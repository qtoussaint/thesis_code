#!/usr/bin/env Rscript
# Compose one binary-GWAS paper figure (TB rifampicin) from a manhattan_short
# directory and a locus-zoom directory.
#
...
#
# Usage:
#   Rscript manhattan_locuszoom_composite_rifampicin.R <manhattan_dir> <locuszoom_dir> [--output-dir <dir>] [--analysis-name <name>]
#
# Output: <output_dir>/figure_<analysis_name>_rifampicin.png
...
parse_args <- function(argv) {
  ...
  if (length(positional) != 2) {
    stop("Usage: Rscript manhattan_locuszoom_composite_rifampicin.R <manhattan_dir> <locuszoom_dir> [--output-dir <dir>] [--analysis-name <name>]")
  }
  list(
...
main <- function() {
  ...
  panel_a_path <- require_file(file.path(manhattan_dir, "manhattan_median_effects.png"))
  panel_b_path <- require_file(file.path(manhattan_dir, "manhattan_exp_abs_median.png"))
  panel_d_path <- require_file(file.path(manhattan_dir, "manhattan_RATE.png"))
  ...
  output_path <- file.path(args$output_dir, paste0("figure_", analysis_name, "_rifampicin.png"))
