...
data {
  ...
  vector[K-1] mic_breakpoints; // MIC breakpoints; log2 is used as cutpoint priors
}
...
model {
  ...
  cutpoints ~ normal(log2(mic_breakpoints), 0.5); // reduces VI instability (cutpoints are not on the same scale as ordcats -- this is a good prior given my integer categories)
  ...
  for (n in 1:N) {
    for (k in 1:(K-1)) {
      ...
      real mu_k = dot_product(variant_matrix[n], beta_variant[,k]) +
                  dot_product(sublineage_matrix[n], beta_sublineage);
